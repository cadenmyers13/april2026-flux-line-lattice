...
class SymmetryCharacterizer:
    ...
    def _symmetry_adapted_filter(self, offset, imshape, n_folds):
        """Generate a symmetry adapted filter (SAF) for given offset and image shape.
    ...
    def _generate_rotated_safs(self, imshape, n_folds):
        ...
        saf_list = [
            self._symmetry_adapted_filter(angle, imshape, n_folds) 
            for angle in angles
        ]
        return np.stack(saf_list, axis=0), angles

    def _calculate_overlap_score(self, image, n_folds):
        """Compute overlap scores between image and rotated SAFs for given n_folds.
    ...
    def apply_annular_mask(self, image, inner_radius=None, outer_radius=None, cx=None, cy=None):
        ...
        cx = cx if cx is not None else (self.cx if self.cx is not None else image.shape[1] // 2)
        cy = cy if cy is not None else (self.cy if self.cy is not None else image.shape[0] // 2)
    ...
    def analyze_dominant_symmetry(self, image, fold_range=range(1, 13), mask_params=None):
        """Find dominant symmetry by comparing max overlap scores across n_folds.
    ...
    def analyze_fft_symmetry(self, image, fold_range=None, mask_params=None):
        ...
        if len(peak_indices) > 0:
            ...
            dominant_n_fold = detected_n_folds_int[0] if len(detected_n_folds_int) > 0 else None
        else:
    ...
    def create_symmetry_fingerprint(self, image, fold_range=range(1, 13), 
                                    mask_params=None, normalize=True):
    ...
    def comprehensive_analysis(self, image, fold_range=range(1, 13), mask_params=None):
        ...
        """
        # Run all analyses
        dominant_results = self.analyze_dominant_symmetry(image, fold_range, mask_params)
        ...
        if dominant_results['dominant_n_fold'] is not None:
            detected_symmetries.append(('dominant_scan', dominant_results['dominant_n_fold']))
    ...
    def plot_dominant_symmetry_scan(self, figsize=(12, 5)):
        ...
        if 'dominant_symmetry' not in self.results:
            print("No dominant symmetry results. Run analyze_dominant_symmetry() first.")
            return
        ...
        ax1.axvline(dominant, color='red', linestyle='--', 
                   label=f'Dominant: {dominant}-fold')
        ax1.legend()
    ...
    def plot_fft_spectrum(self, figsize=(12, 5), max_n_fold=12):
        ...
        if len(fft_res['detected_n_folds']) > 0:
            for n_fold, height in zip(fft_res['detected_n_folds'], 
                                     fft_res['peak_heights']):
                if n_fold <= max_n_fold:
                    ax1.plot(n_fold, height, 'ro', markersize=10)
                    ax1.text(n_fold, height, f'  {n_fold}', 
    ...
    def plot_autocorrelation(self, figsize=(12, 5)):
        ...
        if 'autocorrelation_symmetry' not in self.results:
            print("No autocorrelation results. Run analyze_autocorrelation_symmetry() first.")
            return
        ...
        if len(ac_res['peaks']) > 0:
            ...
            if ac_res['n_fold'] is not None:
                ax1.axvline(peak_lags[0], color='red', linestyle='--',
    ...
    def plot_symmetry_fingerprint(self, figsize=(12, 8), cmap='viridis'):
        ...
        # Create heatmap
        im = ax.imshow(fingerprint, aspect='auto', cmap=cmap, 
    ...
    def plot_comprehensive(self, figsize=(16, 12)):
        ...
        ax2.plot(fft_res['n_fold_values'][mask], fft_res['magnitude'][mask], 
                linewidth=2)
        ax2.set_xlabel('n_fold')
        ...
        im = ax5.imshow(comp_res['fingerprint'], aspect='auto', 
    ...
    def print_summary(self):
        ...
        if ac['n_fold'] is not None:
            ...
            print(f"   Period: {ac['peak_lags'][0]:.2f}° ({int(round(ac['n_fold']))}-fold)")
        else:
